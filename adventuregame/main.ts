...
class Player 
{
    ...
    fuelDecrease()              //fuel decrease function
    {
        let fuel = this.fuel -25
        this.fuel=fuel
    }

    drinkportion()              //fuel decrease function
    {
         this.fuel=100;
    }
...
let player = await inquirer.prompt({             //ask for player name
...
let opponent = await inquirer.prompt({          // ask for opponent 
...
let newname = player.name.split(" ").map((a: any)=> a.charAt(0).toUpperCase()+ 
...
if(p1.fuel <=0 )        //if fuel becomes 0 , player lost the game
{
    ...
    console.log(
    chalk.blueBright.bold.italic('\t\t\tTHANKS FOR PLAYING SSR-CLI-ADVENTURE GAME\t\t ')); 
